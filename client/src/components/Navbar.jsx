  ...
  return (
    <nav className="w-full flex md:justify-center justify-between items-center p-4">
      ...
      md:flex-[0.5] flex-initial justify-center items-center'>
        <h1 className="text-3xl text-white w-32 cursor-pointer">Utopia</h1>
      </div>
      <ul className="text-white md:flex hidden list-none flex-row justify-between items-center flex-initial">
        {[ "Market", "Exchange", "Wallets"].map((item, index) =>(
          <NavBarItem key={item + index} title={item} />
        ))}
        <li className="bg-[#ff184c] py-2 px-7 mx-4 rounded-full cursor-pointer hover:bg-[#05d9e8]">Login</li>
      </ul>
      <div className="flex relative">
        ...
        {toggleMenu && (
          <ul className="z-10 fixed -top-0 -right-2 p-3 w-[70vw] h-screen shadow-2xl md:hidden list-none flex flex-col justify-start items-end rounded-md blue-glassmorphism text-white animate-slide-in">
            ...
            {["Markets", "Exchanges", "Wallets"].map((item, index) => <NavBarItem key={item + index} title={item} classProps="my-2  text-lg" />)}
          </ul>
        )}
      </div>
    </nav>
